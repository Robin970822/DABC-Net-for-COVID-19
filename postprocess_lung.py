...
def remove_small_objects(input_path = '/*',output_path = None,return_nii=True):
    ...
    # niilabel_path=glob(r'C:\Users\yu\Downloads\20_ncov_scan\Label_V5/r*')  # 546
    # niilabel_path = glob(r'I:\gmycode\Lung_ui\2020035365_output\lung/*')
    niilabel_path = glob(input_path+'/*')
    # niilabel_path=glob(r'./Label_V1/c*')  # 546
    niilabel_path.sort()
    ...
    for name in niilabel_path:
        ...
        connected_regions_sorted=sorted(range(len(connected_regions)), key=lambda k: connected_regions[k])  # 返回排序列表的索引（升序）
        ...
        for _tag,rank in enumerate(connected_regions_sorted):  # 误区第一/最大 连通域不是肺，是背景！故_tag要用低三大的比较第二大的！
            # print(num)
            if _tag==2 and connected_regions[rank]<connected_regions[connected_regions_sorted.index(1)]*0.3:  # 比较第二大*0.3
        ...
        if return_nii:
            ...
            # outname = r'I:\gmycode\Lung_ui\2020035365_output\lung_post' + '/' + name.split('\\')[-1]
